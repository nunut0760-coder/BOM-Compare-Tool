...
class BOMCompareGUI:
    ...
    def __init__(self, root):
        ...
        self.root.title(
            "BOM Compare Tool (V1.6.0 by LyShao)"
        )
...
#==========================模糊匹配=========================
    ...
    value = value.replace(",", ".") #逗号和小数点转换
    value = value.replace("µF", "UF") #单位转换
    value = value.replace("ΜF", "UF") #单位转换
    value = value.replace("μF", "UF") #单位转换
    value = re.sub(r'\s+0\s*$', '', value) #删除空格+0+空格
    #value = re.sub(r'\s+', '', value) #删除所有的空格
    value = re.sub(r'(\d+)K(\d+)',r'\1.\2K',value) #4K7变成4.7K
    value = re.sub(r'(\d+\.\d*?[1-9])0+K',r'\1K',value) #删除多余的小数0，4.700K=4.7K
    value = re.sub(r'(\d+)\.0+K',r'\1K',value) #小数为0直接去除，如4.0K=4K
    value = re.sub(r'(\d+)R(\d+)',r'\1.\2R',value) #处理欧姆，1R5=1.5R
    value = re.sub(r'(\d+)M(\d+)',r'\1.\2M',value) #处理兆欧，2M2=2.2M
